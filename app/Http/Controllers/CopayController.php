...
class CopayController extends Controller
{
    ...
    public function report2024802(Request $request)
    {
        date_default_timezone_set('Asia/Manila');
        ...
        if ($doctors != 'All') {
            ...
            SELECT p.name,DATE_FORMAT(s.schedule, '%Y-%m') as schedule,p.attending_doctor,s.free_copay, s.patient_id,s.schedule,s.id,s.doctor
                FROM `schedule` s
    ...
    public function report(Request $request)
    {
        ...
        if ($doctors != 'All') {
            ...
            foreach ($doctors as $key => $value) {
                ...
                $arr['less_wtx'] = number_format($lwt, 2);
                //$net = $value->id == 6 ? $total_Amt * 0.95 : $total_Amt * 0.9;
                $net = $value->id == 5 ? $total_Amt * 0.95 : $total_Amt * 0.9;
                $arr['net'] = number_format($net, 2);
                $data_array[] = $arr;
                ...
                //$tnet = $value->id == 6 ? $total_Amt * 0.95 : $total_Amt * 0.9;
                $tnet = $value->id == 5 ? $total_Amt * 0.95 : $total_Amt * 0.9;
                $data_array_export[] = $arr_export;
                $totalNet += $tnet;
            }

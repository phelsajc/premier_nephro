    ...
    public function report(Request $request)
    {
        ...
        if($doctors!='All'){
            ...
            SELECT p.name,DATE_FORMAT(s.schedule, '%Y-%m'),p.attending_doctor, count(s.patient_id) as cnt, s.patient_id,s.schedule,s.id,s.doctor
                ...
                group by DATE_FORMAT(s.schedule, '%Y-%m'),s.patient_id,s.doctor  order  by p.attending_doctor;
            "); */
            $data =  DB::connection('mysql')->select("
            SELECT p.name,DATE_FORMAT(s.schedule, '%Y-%m'),p.attending_doctor, count(s.patient_id) as cnt, s.patient_id,s.schedule,s.id,s.doctor
                FROM `schedule` s
        ...
        foreach ($data as $key => $value) {
            ...
            $arr_export['Doctor'] =  $checkAttndgDr?$checkAttndgDr->name:'';
            $arr_export['Amount'] = 150;
            $arr_export['Remarks'] = $value->doctor!=$value->attending_doctor?$checkDr->name:'';

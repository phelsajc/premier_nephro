    ...
    public function acpn_report(Request $request)
    {
        ...
        if ($doctors != 'All') {
            ...
            foreach ($getDoctor as $key => $value) {
               ...
               /*  $getDoctor_sessions = DB::connection('mysql')->select("SELECT count(*) as count from phic where date_session between '$fdate' and '$tdate' 
                and remarks like '%$request->batch%' and status='PAID'  and doctor = $value->id "); */
                $arr = array();
                $arr_sharing = array();
        ...
        $getPaidClaims = DB::connection('mysql')->select("
            select * from phic where date_session  and status = 'PAID' and remarks like '%$request->batch%'
        ");
        $data_array = array();
        $data_array_export = array();
        ...
        $data = DB::connection('mysql')->select("Select px.name,p.date_session,p.acpn_no,p.updated_by,p.patient_id,p.doctor,p.updated_dt from phic p 
        left join patients px on px.id = p.patient_id
        where
         p.status = 'PAID'
        and p.remarks like '%$request->batch%'
        group by p.patient_id, p.acpn_no order by px.name");
        foreach ($getPaidClaims as $key => $value) {
            $arr = array();
            $arr_export = array();

            // DB::connection('mysql')->select("SELECT count(*) as count from phic where  remarks like '%$request->batch%' and status='PAID'  and doctor = $value->id ");
            ...
            foreach ($get_dates as $gkey => $gvalue) {
                ...
                /* $data_sessions = DB::connection('mysql')->select("
                SELECT * from phic
                    where  patient_id = '$gvalue->patient_id'
                "); */
                if ($data_sessions) {
                    $data_sessions[0]->status = 'PAID' ? $paid_session++ : 0;
                }
            ...
            $arr['name'] = Helper::patientDetail($value->patient_id)->name;//$value->name;
            ...
            $arr_export['Name'] = Helper::patientDetail($value->patient_id)->name;//$value->name;
